...
/**
 * T003 — FR-005 policy matrix with the plan's default configuration. The policy is a pure object fed
 * by an application {@link PasswordRules} value, so no Spring configuration type is involved.
 */
class PasswordPolicyTest {

    private final PasswordPolicy policy = new PasswordPolicy(new PasswordRules(10, true, true, 128));
    ...
    @Test
    void characterClassRulesAreOptionalWhenConfiguredOff() {
        PasswordPolicy relaxed = new PasswordPolicy(new PasswordRules(4, false, false, 8));

        assertThat(relaxed.violations("1234")).isEmpty();
        assertThat(relaxed.requirements()).containsExactly("At least 4 characters", "At most 8 characters");
    }
