<template>
  <li class="line" data-testid="income-item">
...
import { incomeIcon } from './lineIcons'
import type { ProfileLine } from '../api/profile'

/**
 * One income record, read-only. The amount is the server string; the icon is a presentational
 * hint derived from the source text (`lineIcons.ts`), never a stored value.
 */
...
const icon = computed(() => incomeIcon(props.line.source))
</script>
